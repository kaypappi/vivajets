...
import { TestimonialCarouselProps } from "./testimonial-carousel";
...
const TestimonialMobileWrapper: React.FC<TestimonialCarouselProps> = ({
  slides,
  options,
  className,
}) => {
  ...
  useMotionValueEvent(scrollYProgress, "change", (latest) => {
    const newScrollYProgress=computeClosestMultiple(latest*100,25,slides.length-1)
      ...
      if (scrollRef.current) {
        const childElements =
          scrollRef.current.querySelectorAll<HTMLDivElement>(
            ".thumbnails > .testimonial-mobile-card"
          );
  ...
  const transform = useTransform(scrollYProgress, [0, 0.25], [0, -320], {
    clamp: false,
  });
  ...
  return (
    <>
      <div
        ref={scrollRef}
        style={{height:'400px'  }}
        className={`scroll-container mx-8 rounded-md will-change-transform w-max overflow-y-auto sticky top-40 ${className}`}
      >
        <motion.section
        ...
        >
          <div className="thumbnails sticky top-0 flex gap-x-6 ">
            {slides.map((slide, index) => {
              return (
                <TestimonialMobileCard key={slide.name} slide={slide} className="testimonial-mobile-card w-full min-h-[390px] max-w-[90%]  snap-start"/>
              );
            })}
          </div>
        </motion.section>
...
export default TestimonialMobileWrapper;
